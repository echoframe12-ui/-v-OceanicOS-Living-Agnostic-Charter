...
- GET /attestations/history
- GET /attestations/held
- GET /attestations/attention
...
- `GET /status/digest` is a **signed, portable** digest of the platform's own posture — a canonical snapshot (posture, CVI, source coverage, chain state, held queue, checkpoint head, timestamp) plus an operator-key HMAC over it, so a third party handed the digest can prove it genuinely came from this platform at that time rather than being fabricated. `signed` is false (signature `null`) when no `OCEANICOS_SIGNING_KEY` is set — the platform never claims a signature it can't make. The platform-level analogue of the per-attestation receipt: the receipt proves one entry, the checkpoint seals the ledger head, the digest signs the whole posture (see [DECISIONS/0053](DECISIONS/0053-signed-status-digest.md)). The CLI closes the loop offline — `python oceanic_os.py digest` emits a signed digest from the ledger and `python oceanic_os.py digest --verify FILE` checks one (exit 0/1), no service required; the endpoint and CLI share one `build_payload` so they can't diverge (see [DECISIONS/0054](DECISIONS/0054-offline-status-digest-cli.md)).
...
- `GET /metrics` exposes platform state (CVI, source coverage, held queue, SLA breaches, chain integrity, builds, adapters) in the **Prometheus text exposition format** — scrapeable by any monitoring stack with no custom integration. Aggregate scalars only, public like `/cvi` (see [DECISIONS/0020](DECISIONS/0020-prometheus-metrics.md)).
...
- `GET /attestations/history?subject=…` returns the timeline of attestations for one subject — trust in an artifact over time. Where content-hash lookup groups identical *bytes*, this groups by logical *identity*: every attestation of that exact subject oldest to newest, with `reverified`, `ever_held`, and the confidence trend (`first`/`latest`/`confidence_delta`), so you can see whether an artifact was re-verified and whether it earned its way up (e.g. `held 0.42 → held 0.71 → attested 0.93`). The per-subject companion to the platform-wide CVI history (see [DECISIONS/0046](DECISIONS/0046-subject-attestation-history.md)). The operator console surfaces it as a **Subject // Timeline** panel — trace a subject and see the trend and every re-verification in order (see [DECISIONS/0050](DECISIONS/0050-console-subject-timeline.md)). The console's Stats panel also shows source coverage, and its Receipt panel distinguishes *this entry* (`entry_intact`) from *the ledger* (see [DECISIONS/0051](DECISIONS/0051-console-evidence-and-entry-integrity.md)).
...
- `POST /models/consensus` convenes a 4-member dissent panel — three model heuristics plus a deterministic **rules engine** anchor ("3 competing LLMs + 1 rules engine"). `POST /rules/evaluate` returns the rules engine's verdict *with the named rules that fired and the reason each exists* — the one panel member that explains itself (see [DECISIONS/0017](DECISIONS/0017-rules-engine-panel-anchor.md)). **Dissent is data**: every panel evaluation is recorded in a dissent ledger — `GET /consensus/history` (the trend) and `/consensus/stats` (dissent rate and mean split), with the prompt stored only as a SHA-256, and `oceanicos_dissent_rate` on `/metrics`. Disagreement is a first-class, trended signal, not a transient field (see [DECISIONS/0058](DECISIONS/0058-dissent-ledger.md)). The console's Consensus panel shows the per-evaluation `dissent_score` and the ledger trend (rate and mean split) (see [DECISIONS/0059](DECISIONS/0059-console-dissent-trend.md)).
- Held attestations get a stewardship resolution path (the Arbiter tier's held-review SLA, made real): `GET /attestations/held` (admin) lists them with a `pending`/`released`/`upheld` status; `POST /attestations/<id>/review` records a steward's `release` or `uphold` with a required reason; `GET /attestations/<id>/reviews` is the trail. Reviews are append-only records in their own table — the held attestation is never edited, so the chain stays intact — and a documented release lifts the item out of the CVI's held ratio (see [DECISIONS/0018](DECISIONS/0018-held-review-workflow.md)). `GET /attestations/attention` (admin) ranks the *pending* held items into a prioritized worklist — SLA-breached first, then least confident, then oldest — each carrying the signals behind its rank, so a steward works from the most overdue and least trustworthy down (see [DECISIONS/0056](DECISIONS/0056-attention-queue.md)). The console's Held Review panel surfaces it as a `⚑ work these first` list above the full queue (see [DECISIONS/0057](DECISIONS/0057-console-attention-worklist.md)).
...
- The CLI doubles as a **CI trust gate**: `python oceanic_os.py gate` reads the configured ledger and exits `0`/`1` on a policy, so verification becomes enforcing rather than just observable. By default it requires only an intact chain (like `verify`); `--require-trustworthy` also demands a valid signed checkpoint, `--min-cvi 0.74` fails when the CVI drops below a floor, `--min-sourced 0.9` fails when too little of the record cites evidence, and `--max-held-pending N` / `--no-sla-breach` fail on an unreviewed or SLA-breaching held backlog — so a build fails when trust *regresses* (the record, its confidence, its evidence, or the pace of its review), not only when the chain breaks. Prints `PASS`/`FAIL` with the failing reasons (`--json` for a structured report). The enforce companion to the `verify`/`stats`/`ready` read tools (see [DECISIONS/0047](DECISIONS/0047-ci-trust-gate.md)).
- Evidence discipline is measured, not just assumed: `/attestations/stats` reports `sourced` and `sourced_ratio` — the fraction of attestations that actually cite a source — a second trust axis beside confidence, since a confident attestation with no source trail is a smell the CVI cannot see. The gate can enforce a floor on it (see [DECISIONS/0048](DECISIONS/0048-source-coverage.md)). Source coverage is watched everywhere confidence is: `oceanicos_sourced_ratio` on `/metrics`, `sourced_ratio` on `/status.json`, and a **Sourced** tile on the `/status` board, all from the same `stats()` read (see [DECISIONS/0049](DECISIONS/0049-source-coverage-observability.md)).
