...
| Perpetual drift audits | `drift_audit.py` + `/attestations/audit`, `/attestations/audits` | Each integrity check is recorded to a persistent trail (and one is recorded at every checkpoint), so the record proves it *has been* verified over time, not just that it can be; `oceanicos_last_audit_intact` in `/metrics` alerts on a break; see [DECISIONS/0039](../../DECISIONS/0039-perpetual-drift-audits.md) |
...
| Boot & operator CLI | `oceanic_os.py` + `boot/init.v1` | `oceanic-os --boot …` instantiates the stack from the ratified, hash-attested manifest; the `verify` / `stats` / `ready` subcommands check the ledger and dependencies offline (meaningful exit codes for CI/cron), reusing the same primitives as the HTTP endpoints; see [DECISIONS/0036](../../DECISIONS/0036-operator-cli-subcommands.md) |
...
| Request tracing & logs | `requestlog.py` + before/after hooks | Every response carries an `X-Request-ID` (propagated from the caller or minted, sanitized against log injection) and emits a structured JSON access line (id, method, path, status, actor, latency); see [DECISIONS/0028](../../DECISIONS/0028-request-tracing.md) |
